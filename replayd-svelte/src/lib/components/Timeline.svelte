<script lang="ts">
	...
	let {
		...
		artStyle = 'grooves' as ArtStyle,
		theme = 'light' as Theme,
		frozen = false,
		draggingActive = false,
		hoverSlot = null as number | null,
		highlightSlot = null as number | null,
		wrongSlot = null as number | null,
		onSlotClick,
		onSlotDragOver,
		onSlotDragLeave,
		onSlotDrop,
	...
	<div class="tl-scroll-inner" style="height: {cardSize.h + 20}px">
		{#each Array(slots) as _, i}
			<button
				...
				style="width: {highlightSlot === i || wrongSlot === i || hoverSlot === i
					? cardSize.w + 6
					: draggingActive
						? slotW + 8
						: slotW}px; height: {cardSize.h}px"
			>
				<div
					class="slot-inner"
					style="width: {highlightSlot === i || wrongSlot === i || hoverSlot === i
						? cardSize.w
						: Math.max(slotW - 8, 10)}px; height: {cardSize.h - 8}px; border: 1.5px {hoverSlot ===
						i || highlightSlot === i
						? 'solid'
						: 'dashed'} {wrongSlot === i ? '#0a0a0a' : primary}; background: {highlightSlot === i
						? primary
						: hoverSlot === i
							? 'rgba(10,10,10,0.06)'
							: 'transparent'}; opacity: {highlightSlot === i || wrongSlot === i || hoverSlot === i
						? 1
						: frozen
							? 0.15
							: draggingActive
			...
			{#if i < cards.length}
				<div style="flex-shrink: 0">
					<HitCard
						...
						flipStyle="instant"
						{theme}
						styleExtra="width: {cardSize.w}px; height: {cardSize.h}px"
					/>
				</div>
			{/if}
		{/each}
...
<style>
	...
	.slot {
		flex-shrink: 0;
		transition: width 220ms cubic-bezier(0.7, 0.1, 0.3, 1);
		display: flex;
		align-items: center;
