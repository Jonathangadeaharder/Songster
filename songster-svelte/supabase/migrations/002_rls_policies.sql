...
CREATE POLICY "Authenticated users can join rooms" ON public.players FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
...
CREATE POLICY "Timelines readable by room members" ON public.timelines FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.players p JOIN public.rooms r ON p.room_id = r.id JOIN public.game_state gs ON gs.room_id = r.id WHERE p.user_id = auth.uid())
);
